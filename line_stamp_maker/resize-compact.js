...
// メイン処理
async function main() {
    console.log('🎨 LINEスタンプ用リサイズツール（余白なし・コンパクト版）');
    console.log('===============================================');
    console.log('※ 最大サイズ(370x320)に収まるようにリサイズします（余白なし）');
    ...
    try {
        ...
        console.log(`📏 サイズ: 最大 ${CONFIG.finalSize.width}×${CONFIG.finalSize.height}px（余白なし）`);
...
async function resizeImage(filename) {
    ...
    await sharp(inputPath)
        .resize(CONFIG.finalSize.width, CONFIG.finalSize.height, {
            fit: 'inside', // アスペクト比維持、指定サイズ内に収める（余白なし）
            withoutEnlargement: false // 小さい画像は拡大する（必要に応じてtrueに変更可）
        })
        .png()
        .toFile(outputPath);
}

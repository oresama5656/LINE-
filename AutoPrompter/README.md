...
- **Stopボタン**: GUIの停止ボタン（推奨）
- **Ctrl+C**: コマンドラインから中断（CLIモードのみ）
- ~~**マウスを画面左上角に移動**: PyAutoGUIのフェイルセーフ機能~~ **無効化済み**
  - 処理中のマウス操作を可能にするため、フェイルセーフ機能は無効化されています
  - GUIのStopボタンまたはCtrl+Cで停止してください
...
**エラー例**: `特殊文字エラー: 'é' (cp932でエンコードできない文字が含まれています)`

**原因**: CSVファイルに`café`, `naïve`などのcp932でエンコードできない特殊文字が含まれています

**解決方法**:
- CSVファイル内の特殊文字を通常のASCII文字に変更してください
  - `café` → `cafe`
  - `naïve` → `naive`
  - `résumé` → `resume`
...
- 複数エンコーディング対応（UTF-8 with BOM, UTF-8, CP932, Shift-JIS）

## 変更履歴

### 2025-10-25
- ✅ **CSVエンコーディング対応の改善**: UTF-8 with BOMを優先的に処理
- ✅ **特殊文字エラーメッセージの改善**: エラー原因と解決方法を明示
- ✅ **PyAutoGUI FAILSAFEの無効化**: 処理中のマウス操作を可能に
- ✅ **エラーログの改善**: CSV Mode時に利用可能な列名を表示

### 初期リリース
- ChatGPT Web版への自動プロンプト送信機能
- GUI/CLIモード対応
- Prefix/Suffix機能
- リトライ機能、Dry-runモード
